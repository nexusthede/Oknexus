...
module.exports = {
  name: "voicemaster",
  description: "VoiceMaster interface and commands",
...
<:vc_claim:1477559856394403942> - Claim ownership of your voice channel
    ...
    if (command === "voicechannel") {
      ...
      try {
        await performVoiceChannelAction(action, member, channel, args.slice(1));
      } catch (err) {
        console.error(err);
...
// --------------------------
// Function to perform voice channel actions
// --------------------------
...
// --------------------------
module.exports.setupVoiceListeners = (client) => {
  client.on("voiceStateUpdate", async (oldState, newState) => {
    const member = newState.member;

    // ------------- JOIN TO CREATE (VoiceMaster) -------------
    if (newState.channelId === config.JOIN_TO_CREATE_ID) {
      try {
        ...
        // Auto-delete empty voice channel after 30s
        const checkEmpty = setInterval(async () => {
          if (!vc.deleted && vc.members.size === 0) {
